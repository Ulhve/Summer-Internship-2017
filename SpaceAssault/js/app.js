...
var megaliths = [
		...
		sprite: new Sprite('img/sprites_02.png', [0, 212], [60, 54])},
		{pos: [Math.random() * (canvas.width - 50),
                  Math.random() * (canvas.height - 50)],
		sprite: new Sprite('img/sprites_02.png', [0, 275], [54, 42])},   
	];
var manna = [];
...
var isGameOver;
var terrainPattern;
var lastMannaAdd = Date.now();
var mannaAddPeriod = 4000;
var mannaMaxCount = 10;
...
function update(dt) {
	...
	if ((Date.now() - lastMannaAdd > mannaAddPeriod) && (manna.length < mannaMaxCount)) {
		manna.push({
			pos: [Math.random() * (canvas.width - 50),
				Math.random() * (canvas.height - 50)],
			sprite: new Sprite('img/sprites_02.png', [4, 320], [52, 56], 6, [0, 1, 1, 1, 0])
		});
		lastMannaAdd = Date.now();
    }
...
function handleInput(dt) {
    if(input.isDown('DOWN') || input.isDown('s')) {
		...
		if (isCollisionMegaliths(player)) {
			player.pos[1] -= playerSpeed * dt;
		}
    }
...
function updateEntities(dt) {
    ...
    // Update all the enemies
		for(var i=0; i<enemies.length; i++) {
			...
			var posEnemy = enemies[i].pos;
			var sizeEnemy = enemies[i].sprite.size;
...
function isCollisionMegaliths(entity) {

    for(var i=0; i<megaliths.length; i++) {
        if (boxCollides(entity.pos, entity.sprite.size, megaliths[i].pos, megaliths[i].sprite.size)) {
            return true;
        }
    }
    return false;
...
function checkCollisions() {
	...
	for(var k=0; k<bullets.length; k++) {
        ...
        if(isCollisionMegaliths(bullets[k])) {
			
            addExplosion(bullets[k].pos);
            bullets.splice(k, 1);
            k--;
        }
    }
